...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['admit_officer'])) {
    ...
    try {
        ...
        if ($notif_details) {
            ...
            header('Location: notifications.php?msg=admitted');
            exit();
        }
    } catch (PDOException $e) {
        header('Location: notifications.php?msg=error');
        exit();
    }
}

// Handle Report Officer (Officer did not report)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['report_officer'])) {
    $notification_id = intval($_POST['notification_id']);
    ...
    try {
        ...
        if ($notif_details) {
            ...
            if (class_exists('CacheManager')) { CacheManager::delete('dashboard_analytics'); }
            header('Location: notifications.php?msg=reported');
            exit();
        }
    } catch (PDOException $e) {
        header('Location: notifications.php?msg=error');
...
$notifications = getNotifications($userZoneNames, 100);
...
<div class="page-title">
    ...
    <div>
        ...
        <?php if ($unreadCount > 0): ?>
            <a href="?mark_all_read=1" class="btn-sm btn-success" style="margin-left:0.5rem;"><i class="fas fa-check-double"></i> Mark All Read</a>
        <?php endif; ?>
    </div>
</div>
            ...
            <div id="notifContainer">
                ...
                <div class="notification-item <?php echo $itemClass; ?>" 
                        ...
                        <div class="action-group">
                            ...
                            <form method="POST" class="admit-form" onsubmit="return confirm('Report this officer as not reported?');">
                                ...
                                <button type="submit" name="report_officer" class="btn-sm btn-report">
                                    <i class="fas fa-user-times"></i> Report Not Reported
                                </button>
                            </form>
                        </div>
                        <?php endif; ?>
                    ...
                    <div class="notification-actions">
                        ...
                        <?php endif; ?>
                        <a href="search.php?serviceNo=<?php echo urlencode($notif['serviceNo']); ?>" class="btn-sm btn-outline" title="View Officer Profile"><i class="fas fa-eye"></i> View</a>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
...
function filterItems(type, btn) {
    ...
    items.forEach(item => {
        ...
        if (type === 'all') {
            ...
            item.style.display = (status === 'admitted') ? 'flex' : 'none';
        } else if (type === 'reported') {
            item.style.display = (status === 'reported' || status === 'auto_reported') ? 'flex' : 'none';
        }
    });
}
